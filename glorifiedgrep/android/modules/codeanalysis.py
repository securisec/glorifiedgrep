...
class _CodeAnalysis(_AndroidCore, _GrepConstants):
    ...
    def code_sql_java_implementation(self, show_code: bool = False) -> GreppedOut:
        ...
        match["query"] = self._process_match(
            self._run_rg(regex=java_query, code=show_code)
        )
        match["insert"] = self._process_match(
            self._run_rg(regex=java_insert, code=show_code)
        )
        match["update"] = self._process_match(
            self._run_rg(regex=java_update, code=show_code)
        )
        match["delete"] = self._process_match(
            self._run_rg(regex=java_delete, code=show_code)
        )
    ...
    def code_tcp_sockets(self, show_code: bool = False) -> GreppedOut:
        ...
        """
        regex = (
            r"^([^import].+)(\bSocket\(.*\)|MulticastSocket\(.*\)|ServerSocket\(.*\))"
        )
        match = self._run_rg_and_process(regex=regex, code=show_code)
    ...
    @_logger
    def code_search(
        self, regex: str, rg_options: str = "", show_code: bool = False
    ) -> GreppedOut:
        """
        ...
        g = self._run_rg(regex=regex, rg_options=rg_options, code=show_code)
        match = self._process_match(g)
        self.log_debug("")
        return GreppedOut(match)
    ...
    def code_intent_filters(self, show_code: bool = False) -> GreppedOut:
        ...
        for filters in self._RG_INTENT_EXTRAS:
            match[filters] = self._run_rg_and_process(regex=filters, code=show_code)
    ...
    @_logger
    def code_android_contacts_content_provider(
    ...
    def code_send_sms_text(self, show_code: bool = False) -> GreppedOut:
        ...
        """
        regex = self._IGNORE_IMPORTS + r"(sendTextMessage|sendMultipartTextMessage)"
        match = self._run_rg_and_process(regex=regex, code=show_code)
    ...
    def code_boot_completed_persistance(self, show_code: bool = False) -> GreppedOut:
        ...
        match = self._run_rg_and_process(
            regex=regex, code=show_code, path=f"{self._output_dir}"
        )
    ...
    @_logger
    def code_apache_http_other_request_methods(
        self, show_code: bool = False
    ) -> GreppedOut:
        """
    ...
    def code_imports(self, class_name: str) -> list:
        ...
        """
        return (
            Ripgrepy(fr"import.+{class_name}\b", self._java_sources)
            .files_with_matches()
            .run()
            .as_string()
            .splitlines()
