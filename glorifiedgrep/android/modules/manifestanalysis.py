...
class _ManifestAnalysis(_AndroidCore, _AppAnalysisConstants):
    ...
    def all_manifest_analysis(self) -> dict:
        ...
        :rtype: dict
        """
        methods = [p for p in vars(_ManifestAnalysis).keys() if not p.startswith("_")]
    ...
    def manifest_main_activity(self) -> dict:
        ...
        for i in range(len(activities)):
            try:
                if (
                    activities[i]["intent-filter"]["action"]["name"]
    ...
    def manifest_min_sdk(self) -> int:
        ...
        """
        try:
            p = self._parse_manifest_xml()["uses-sdk"]["minSdkVersion"]
    ...
    def manifest_android_version(self) -> dict:
        ...
        min_sdk = [
            x
            for x in _AppAnalysisConstants._ANDROID_VERSIONS
            if x["api"] == str(self.manifest_min_sdk())
        ][0]
        target_sdk = [
            x
            for x in _AppAnalysisConstants._ANDROID_VERSIONS
            if x["api"] == str(self.manifest_target_sdk())
        ][0]
    ...
    def manifest_uses_library(self) -> list:
        ...
        """
        try:
            p = self._parse_manifest_xml()["application"]["uses-library"]
    ...
    def manifest_secrets(self) -> list:
        ...
        """
        g = self._run_rg(
            regex=self._RG_MANIFEST_SECRET,
            code=True,
            rg_options="-iH",
