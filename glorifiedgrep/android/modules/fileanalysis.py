...
class _FileAnalysys(_AndroidCore):
    def all_file_analysis(self):
        ...
        >>> a.all_file_analysis()
        """
        methods = [p for p in vars(_FileAnalysys).keys() if not p.startswith("_")]
    ...
    def file_get_file_types(self, describe: bool = False, exclude: list = [""]) -> dict:
        ...
        for p in self._get_paths_from_unzipped(self._unzipped_path):
            ...
            if key in found:
                if describe:
                    found[key].append({"file": p, "description": magic.from_file(p)})
                else:
                    found[key].append(p)
            else:
                found[key] = []
        ...
        if exclude[0] != "":
            ...
            if describe:
                return {
                    k: list(
                        x for x in found[k] if not any(s in x["file"] for s in exclude)
                    )
                    for k in found
                }
            else:
                return {
                    k: list(x for x in found[k] if not any(s in x for s in exclude))
                    for k in found
                }
        return dict(found)
    ...
    def file_get_java_classes(self) -> list:
        ...
        for i in self._get_paths_from_unzipped(
            f"{self._java_sources}", glob_pattern="**/*.java"
        ):
    ...
    def file_react_app(self) -> bool:
        ...
        >>> a.file_react_app()
        """
        for f in self._get_paths_from_unzipped(
            path=f"{self._output_dir}/unzipped", glob_pattern="**/*"
        ):
    ...
    def file_other_langs(self) -> dict:
        ...
        """
        self.log_debug("")
        return {"kivy": self.file_kivy_app(), "react": self.file_react_app()}
    ...
    def file_xml_files(self) -> list:
        ...
        """
        found = [
            self._re_remove_dir_prefix(str(x))
            for x in self._get_paths_from_unzipped(
    ...
    def file_js_files(self) -> list:
        ...
        """
        found = [
            re.sub(f"{self._output_dir}/", "", str(x))
            for x in self._get_paths_from_unzipped(
    ...
    def file_res_strings(self) -> list:
        ...
        with open(f"{self._output_dir}/resources/res/values/strings.xml", "r") as f:
            data = json.loads(json.dumps(xmltodict.parse(f.read(), attr_prefix="")))
