...
class Logger:
    ...
    def __init__(self, verbose: bool = True, log_path: str = None):
        ...
        stream_formatter = logging.Formatter(
            "%(asctime)-15s %(levelname)-8s %(module)s.%(funcName)s:%(lineno)-4s %(message)s",
            datefmt="%A %d %H:%M:%S",
        )
        ...
        logging.debug("Python version: {}".format(platform.python_version()))
        # logs OS relevant information
        system_type = platform.system()
        if system_type == "Linux":
            logging.debug("OS information: {}".format((platform.linux_distribution())))
